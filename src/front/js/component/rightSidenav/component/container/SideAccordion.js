...
export const SideAccordion = ({ extraClass, title, show, handler, collapsed }) => {
    const startDate = new Date().toLocaleDateString();
    const endDate = new Date().toLocaleDateString();
    const eventdate = `${startDate}${startDate === endDate ? "" : ` ${endDate}`}`;

    return (
        <div className={"accordion " + extraClass} id="accordionExample">
            <div className="accordion-item overflow-hidden border-0">
